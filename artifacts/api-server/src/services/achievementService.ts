import { findCompletedAnalyses } from "../repositories/achievementRepository";
...
export async function getAchievements(userId: number) {
  const rows = await findCompletedAnalyses(userId);
  ...
  let streak = 0;
  for (let i = 0; i < 366; i++) {
    if (daySet.has(today.getTime() - i * 86_400_000)) { streak++; }
    else if (i > 0) { break; }
  }
  ...
  for (const r of rows) sportCounts.set(r.sport.toLowerCase(), (sportCounts.get(r.sport.toLowerCase()) ?? 0) + 1);
  ...
  const achievements = [
    ...
    mk("improve-10", "Rising Star",  "Improve your overall score by 10+ points", "arrow-up-circle", "Growth", Math.round(Math.max(0, improvement)), 10),
    mk("improve-20", "Breakthrough", "Improve your overall score by 20+ points", "trending-up",     "Growth", Math.round(Math.max(0, improvement)), 20),
