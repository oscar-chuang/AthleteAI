/**
 * Unit test: rest-day tooltip dismiss interaction.
 *
 * Renders the full HomeScreen with the rest-day tooltip visible (profile has
 * rest days, rest_day_tooltip_dismissed key absent), then taps the dismiss
 * button and asserts:
 *   1. AsyncStorage.setItem("rest_day_tooltip_dismissed", "true") is called.
 *   2. The tooltip element is removed from the rendered tree.
 *
 * This closes the same timer-cancellation gap that shareHintAutoDismiss.test.tsx
 * guards for the share hint: if an auto-dismiss timer is ever added to
 * dismissRestDayTooltip, this test will catch any double-write or missed-cleanup
 * regression before it ships.
 *
 * Mocking strategy mirrors restDayTooltipOnce.test.tsx exactly — useFocusEffect
 * is captured and fired manually so the component loads its data and renders the
 * weekly-dot row that hosts the tooltip.
 */
...
beforeEach(() => {
  ...
  // Key absent: tooltip has not been dismissed yet.
...
describe("HomeScreen — rest-day tooltip dismiss interaction", () => {
  it("writes rest_day_tooltip_dismissed='true' and removes the tooltip from the tree when the dismiss button is tapped", async () => {
    const { queryByTestId, getByTestId, queryByText } = render(<HomeScreen />);
    await simulateFocus();

    // Tooltip must be visible before the tap.
    ...
    // 1) AsyncStorage must record the dismissal.
    expect(AsyncStorage.setItem).toHaveBeenCalledWith(
      "rest_day_tooltip_dismissed",
      "true",
    );

    // 2) Tooltip must be gone from the rendered tree.
