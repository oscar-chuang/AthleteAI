...
(function(){
  ...
  function drawSkeleton(res){
    ...
    KEYS.forEach(function(i){
      ...
      liveCtx.fillStyle='#2F7BFF';liveCtx.fill();
    });
  }
...
export default function SkeletonScreen() {
    ...
    return (
      <View style={ss.chipRow}>
        {arr.map((j) => {
          ...
          const color = typeof lvl === "number" ? RISK_COLORS[lvl] : "#2F7BFF";
  ...
  function buildNextStepCueForScreen(
    drill: DrillRecord | string,
    kind: "injury" | "performance",
    joints?: JointKey[],
  ): string {
    return buildNextStepCue(drill, kind, joints, jointTrendsData);
  }
        ...
        {expanded && (
          <View style={{ gap: 11, marginTop: 2 }}>
            ...
            {tip.drill ? (() => {
              ...
              const accentColor = kind === "performance" ? "#2F7BFF" : "#FF6B35";
              return (
                  ...
                  {completedDrills.has(tip.id) && (
                    <View style={ss.nextStepCard}>
                      ...
                      <Text style={ss.nextStepCue}>
                        {buildNextStepCueForScreen(drill, kind, tjoints)}
                      </Text>
                      <TouchableOpacity
  ...
  const heroBlock = !videoUri ? null : preparing ? (
    ...
    <View>
      ...
      <View style={{ width: heroBoxW, height: heroBoxH }}>
        ...
        {scrubTick && scrubTick.lm.length > 0 && (() => {
          ...
          const bones = SCRUB_CONNECTIONS.map(([a, b], i) => {
            ...
            const color = risk >= 2 ? "#EF4444aa" : risk >= 1 ? "#FF6B35aa" : "#2F7BFFaa";
          ...
          const dots = SCRUB_KEY_LM.map((idx) => {
            ...
            const color = risk >= 2 ? "#EF4444" : risk >= 1 ? "#FF6B35" : "#f8fafc";
            const r = risk >= 1 ? 6 : 4;
            return (
              <G key={`sd${idx}`}>
      ...
      <View style={ss.heroBanner} pointerEvents="none">
        ...
        {(() => {
          const qColor = heroScanQuality === "high" ? "#22C55E" : heroScanQuality === "medium" ? "#FF6B35" : "#6b7280";
          return (
      ...
      {frameTicks.length > 0 && (
        <View style={ss.scrubberWrap}>
          <View style={ss.scrubberHeader}>
            <Feather name="film" size={10} color="#2F7BFF" />
          ...
          >
            ...
            {frameTicks.map((tick, idx) => {
              ...
              const color = Object.values(tick.jr).some((jr) => (jr?.lvl ?? 0) >= 2) ? "#EF4444" : "#FF6B35";
              return (
                <TouchableOpacity
                  key={idx}
          ...
          {riskTickPositions.length > 0 ? (
            <View style={ss.scrubberNavRow}>
              <TouchableOpacity
                ...
                activeOpacity={0.7}
              >
                <Feather name="chevron-left" size={13} color="#2F7BFF" />
              </TouchableOpacity>
              ...
              <TouchableOpacity
                ...
                activeOpacity={0.7}
              >
                <Feather name="chevron-right" size={13} color="#2F7BFF" />
              </TouchableOpacity>
            </View>
          ) : (
    ...
    <View style={ss.root}>
      ...
      <View style={[ss.header, { paddingTop: topPad + 8 }]}>
        ...
        {videoUri && scanDone && hasFrameTicks && (
          <TouchableOpacity
            style={[ss.headerBtn, { backgroundColor: "#2F7BFF22", borderColor: "#2F7BFF55" }]}
            onPress={() => router.push(`/analysis/live/${id}` as any)}
            activeOpacity={0.8}
          >
            <Feather name="play-circle" size={13} color="#2F7BFF" />
            <Text style={[ss.headerBtnText, { color: "#2F7BFF" }]}>Breakdown</Text>
          </TouchableOpacity>
        )}
        {videoUri && scanDone && (
      ...
      >
        ...
        {scanDone && hero && flaggedJoints.length > 0 && (
          <View style={ss.heroChips}>
            ...
            <View style={ss.tapHintRow}>
              <Feather name={Object.keys(jointTrendsData?.joints ?? {}).length > 0 ? "bar-chart-2" : "crosshair"} size={10} color="#2F7BFF" />
        ...
        {videoUri && groundedReady && !refining && performanceTips.length > 0 && (
          <View style={ss.tipSection}>
            <View style={ss.tipLabelRow}>
              <Feather name="zap" size={10} color="#2F7BFF" />
              <Text style={[ss.sectionLabel, { color: "#2F7BFFaa" }]}>PERFORMANCE COACHING</Text>
            </View>
            {(() => {
          ...
          <View style={ss.sourcesSection}>
            ...
            {showSources && (
              <View style={ss.sourcesCard}>
                ...
                <Text style={[ss.sourcesGroupLabel, { color: "#2F7BFFaa", marginTop: 14 }]}>⚡ Running · Cycling · Triathlon</Text>
                {([
                  ...
                  <View key={i} style={ss.sourceRow}>
                    <Text style={[ss.sourceNum, { color: "#2F7BFF" }]}>{i + 1}</Text>
                    <View style={{ flex: 1 }}>
                      <Text style={ss.sourceRef}>{s.ref} <Text style={ss.sourceYear}>({s.year})</Text></Text>
                      <Text style={ss.sourceTitle}>{s.title}</Text>
                      <Text style={ss.sourceJournal}>{s.journal}{s.detail ? `, ${s.detail}` : ""}</Text>
                ...
                <Text style={[ss.sourcesGroupLabel, { color: "#2F7BFFaa", marginTop: 14 }]}>⚡ Weightlifting · Powerlifting · CrossFit</Text>
                {([
                ...
                <Text style={[ss.sourcesGroupLabel, { color: "#2F7BFFaa", marginTop: 14 }]}>⚡ Basketball · Volleyball</Text>
                {([
                ...
                <Text style={[ss.sourcesGroupLabel, { color: "#2F7BFFaa", marginTop: 14 }]}>⚡ Soccer · Football · Rugby · Lacrosse</Text>
                {([
                ...
                <Text style={[ss.sourcesGroupLabel, { color: "#2F7BFFaa", marginTop: 14 }]}>⚡ Tennis · Baseball · Softball</Text>
                {([
                ...
                <Text style={[ss.sourcesGroupLabel, { color: "#2F7BFFaa", marginTop: 14 }]}>⚡ Swimming · Gymnastics · Diving</Text>
                {([
                ...
                <Text style={[ss.sourcesGroupLabel, { color: "#2F7BFFaa", marginTop: 14 }]}>⚡ Fencing · Boxing · Wrestling · Martial Arts</Text>
                {([
                ...
                <Text style={[ss.sourcesGroupLabel, { color: "#2F7BFFaa", marginTop: 14 }]}>⚡ Rowing · Hockey · Badminton · Golf</Text>
                {([
                ...
                <Text style={[ss.sourcesGroupLabel, { color: "#2F7BFFaa", marginTop: 14 }]}>⚡ Track & Field · Skiing · Snow Sports</Text>
                {([
                ...
                <View style={ss.aiDisclaimer}>
                  <Feather name="cpu" size={11} color="#2F7BFF" />
...
function makeStyles(c: ReturnType<typeof useColors>) {
  return StyleSheet.create({
    ...
    header:        { flexDirection: "row", alignItems: "center", paddingHorizontal: 20, paddingBottom: 12, borderBottomWidth: 1, borderBottomColor: c.border, gap: 12 },
    ...
    headerThumb:   { width: 52, height: 36, borderRadius: 7, borderWidth: 1, borderColor: c.border },
    headerTitle:   { fontSize: 14, fontFamily: "Inter_600SemiBold", color: c.textPrimary, textTransform: "capitalize" },
    headerBtn:     { flexDirection: "row", alignItems: "center", gap: 6, backgroundColor: c.primary, borderRadius: 20, paddingHorizontal: 12, paddingVertical: 6 },
    headerBtnText: { fontSize: 12, color: "#fff", fontFamily: "Inter_600SemiBold" },
    sectionLabel:  { fontSize: 10, color: c.textTertiary, fontFamily: "Inter_600SemiBold", letterSpacing: 1.5 },

    heroSlot:      { backgroundColor: c.background, alignItems: "center", justifyContent: "center", gap: 9, paddingHorizontal: 30 },
    scanTitle:     { fontSize: 14, color: c.textPrimary, fontFamily: "Inter_600SemiBold", textAlign: "center" },
    scanSub:       { fontSize: 12, color: c.textTertiary, fontFamily: "Inter_400Regular", textAlign: "center" },
    progTrack:     { width: "70%", height: 4, borderRadius: 3, backgroundColor: "rgba(28,28,46,0.85)", overflow: "hidden", marginTop: 4 },
    progFill:      { height: "100%", borderRadius: 3, backgroundColor: c.primary },
    preparingText: { fontSize: 12, color: c.textTertiary, fontFamily: "Inter_400Regular" },
    ...
    tipCard:       { backgroundColor: c.surface2, borderRadius: 14, borderWidth: 1, padding: 14, gap: 10, marginBottom: 10 },
    ...
    tipCategory:   { fontSize: 10, color: c.primary, fontFamily: "Inter_600SemiBold", letterSpacing: 0.5, textTransform: "uppercase" },
    tipTitle:      { fontSize: 14, fontFamily: "Inter_600SemiBold", color: c.textPrimary, marginTop: 1 },
    tipDesc:       { fontSize: 13, color: c.textSecondary, fontFamily: "Inter_400Regular", lineHeight: 19 },
    drillBox:        { borderRadius: 10, borderWidth: 1, padding: 11, gap: 6 },
    drillLabel:      { fontSize: 9, fontFamily: "Inter_700Bold", letterSpacing: 1 },
    drillText:       { fontSize: 12, color: c.textPrimary, fontFamily: "Inter_400Regular", lineHeight: 17 },
    drillStructured: { gap: 8 },
    drillName:       { fontSize: 13, color: c.textPrimary, fontFamily: "Inter_600SemiBold" },
    drillMeta:       { flexDirection: "row", gap: 8 },
    drillMetaPill:   { backgroundColor: c.background, borderRadius: 8, paddingHorizontal: 10, paddingVertical: 5, gap: 2, alignItems: "center" },
    drillMetaLabel:  { fontSize: 8, fontFamily: "Inter_700Bold", letterSpacing: 0.8 },
    drillMetaValue:  { fontSize: 13, color: c.textPrimary, fontFamily: "Inter_600SemiBold" },
    drillCueRow:     { flexDirection: "row", alignItems: "flex-start", gap: 7, paddingTop: 2 },
    drillCueText:    { flex: 1, fontSize: 12, color: c.textSecondary, fontFamily: "Inter_400Regular", lineHeight: 17, fontStyle: "italic" },
    ...
    drillDoneBadge:  { flexDirection: "row", alignItems: "center", gap: 4, backgroundColor: "#0D1610", borderRadius: 10, paddingHorizontal: 8, paddingVertical: 3, borderWidth: 1, borderColor: "#22C55E33" },
    drillDoneBadgeText: { fontSize: 10, color: "#22C55E", fontFamily: "Inter_700Bold", letterSpacing: 0.3 },
    nextStepCard:       { marginTop: 8, backgroundColor: "#0A160D", borderRadius: 10, borderWidth: 1, borderColor: "#22C55E33", padding: 10, gap: 6 },
    ...
    nextStepAskBtn:     { flexDirection: "row", alignItems: "center", gap: 6, backgroundColor: "#0D1610", borderRadius: 8, paddingVertical: 7, paddingHorizontal: 10, borderWidth: 1, borderColor: "#22C55E55", alignSelf: "flex-start", marginTop: 2 },
    ...
    okCard:        { flexDirection: "row", gap: 12, alignItems: "center", backgroundColor: "#0D1610", borderWidth: 1, borderColor: "#22C55E33", borderRadius: 14, padding: 14 },
    ...
    whyBox:        { backgroundColor: c.surface1, borderRadius: 8, borderWidth: 1, borderColor: c.borderStrong, paddingHorizontal: 10, paddingVertical: 8, gap: 3 },
    whyLabel:      { fontSize: 9, color: c.primary, fontFamily: "Inter_700Bold", letterSpacing: 1 },
    whyText:       { fontSize: 12, color: c.textSecondary, fontFamily: "Inter_400Regular", lineHeight: 17 },
    ...
    tapHint:       { fontSize: 10, color: c.primary, fontFamily: "Inter_600SemiBold" },

    miniWrap:      { borderRadius: 12, overflow: "hidden", borderWidth: 1, borderColor: c.border, alignSelf: "center" },
    ...
    readingPill:   { flexDirection: "row", alignItems: "center", gap: 5, borderWidth: 1, borderRadius: 8, paddingHorizontal: 8, paddingVertical: 5, backgroundColor: c.surface1 },
    ...
    citeText:      { flex: 1, fontSize: 10, color: c.textTertiary, fontFamily: "Inter_400Regular", fontStyle: "italic", lineHeight: 14 },
    askBtn:        { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, backgroundColor: c.primary, borderRadius: 12, paddingVertical: 11, marginTop: 2 },
    askBtnText:    { fontSize: 13, color: "#fff", fontFamily: "Inter_600SemiBold" },

    refiningCard:  { flexDirection: "row", gap: 12, alignItems: "center", backgroundColor: c.surface2, borderWidth: 1, borderColor: c.primary + "33", borderRadius: 14, padding: 14 },
    refiningTitle: { fontSize: 13, color: c.textPrimary, fontFamily: "Inter_600SemiBold" },
    refiningBody:  { fontSize: 11, color: c.textTertiary, fontFamily: "Inter_400Regular", marginTop: 2, lineHeight: 16 },
    ...
    sourcesToggleText:   { fontSize: 11, color: c.textTertiary, fontFamily: "Inter_400Regular", flex: 1 },
    sourcesCard:         { backgroundColor: c.surface1, borderRadius: 12, borderWidth: 1, borderColor: c.border, padding: 14, marginTop: 8, gap: 12 },
    sourcesHeading:      { fontSize: 13, fontFamily: "Inter_700Bold", color: c.textPrimary },
    sourcesSubheading:   { fontSize: 11, color: c.textTertiary, fontFamily: "Inter_400Regular", lineHeight: 16, fontStyle: "italic" },
    ...
    groupDividerLine: { flex: 1, height: 1, backgroundColor: c.borderStrong },
    groupDividerText: { fontSize: 9, color: c.textTertiary, fontFamily: "Inter_600SemiBold", letterSpacing: 1.5 },
    sourceRow:           { flexDirection: "row", gap: 10, alignItems: "flex-start" },
    sourceNum:           { fontSize: 11, color: c.primary, fontFamily: "Inter_700Bold", width: 16, paddingTop: 1 },
    sourceRef:           { fontSize: 11, color: c.textPrimary, fontFamily: "Inter_600SemiBold" },
    sourceYear:          { fontSize: 11, color: c.textTertiary, fontFamily: "Inter_400Regular" },
    sourceTitle:         { fontSize: 11, color: c.textSecondary, fontFamily: "Inter_400Regular", lineHeight: 15, marginTop: 1 },
    sourceJournal:       { fontSize: 10, color: c.textTertiary, fontFamily: "Inter_400Regular", fontStyle: "italic", marginTop: 1 },
    aiDisclaimer:        { flexDirection: "row", gap: 7, alignItems: "flex-start", backgroundColor: c.surface2, borderRadius: 8, padding: 10, marginTop: 4 },
    aiDisclaimerText:    { fontSize: 10, color: c.textTertiary, fontFamily: "Inter_400Regular", lineHeight: 15, flex: 1 },
    noVideo:       { flex: 1, alignItems: "center", justifyContent: "center", gap: 10, paddingHorizontal: 40, paddingTop: 60 },
    noVideoText:   { fontSize: 14, color: c.textSecondary, fontFamily: "Inter_600SemiBold", textAlign: "center" },
    noVideoSub:    { fontSize: 12, color: c.textTertiary, fontFamily: "Inter_400Regular", textAlign: "center" },

    scrubberWrap:     { backgroundColor: c.surface1, borderTopWidth: 1, borderTopColor: c.border, paddingHorizontal: 16, paddingTop: 11, paddingBottom: 13 },
    scrubberHeader:   { flexDirection: "row", alignItems: "center", gap: 6, marginBottom: 8 },
    scrubberLabel:    { fontSize: 9, color: c.primary, fontFamily: "Inter_700Bold", letterSpacing: 1.2, flex: 1 },
    scrubberTime:     { fontSize: 10, color: c.textTertiary, fontFamily: "Inter_500Medium" },
    scrubberTrack:    { height: 28, justifyContent: "center", position: "relative" },
    scrubberFill:     { height: 4, borderRadius: 2, backgroundColor: c.borderStrong, overflow: "hidden", flexDirection: "row" },
